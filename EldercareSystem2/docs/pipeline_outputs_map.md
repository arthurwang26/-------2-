...
# EldercareSystem Pipeline 輸出對照表 (V5.1)
...
- `outputs3/`：**目前 (V5.1 終極旗艦版)** 啟動的測試結果存放區。以下路徑皆以 `outputs3/` 為基準。

## 1. 大語言模型對比除錯報告 (llm_reports/)

這也是本系統 V5.1 最核心的學術亮點，所有 LLM 報告都會自動比對雙軌 HOI 的結果。
...
*   **`llm_reports/final_report.md`**：
    *   長照監控系統全局最終報告。包含跨天行為推理、異常預測、雙軌 HOI 與 MotionBERT (DSTformer) 感測器盲區分析等終極統整。
...
*   這些影片直接將 YOLO BBox、RTMPose 骨架、以及 MotionBERT (DSTformer)/Dual-HOI 的預測結果壓制在畫面上，適合做為學術展示 (Demo) 的直觀證據。
