# 老人日照中心多模態行為分析系統 (V4.4 終極旗艦版)

本專案已徹底進化為**「具備雙 HOI 架構與真實 AI 預訓練模型的高階邊緣運算稽核系統」**。

系統運行於全新的 `EldercareSystem/` 目錄下。V4.4 最大的創舉在於：我們不僅將大型語言模型 (LLM) 升級為「系統除錯稽核員」，更在極限 VRAM (4GB) 限制下，成功載入了 **真實預訓練動作辨識 (MMAction2)** 與 **雙軌人機互動 (Dual-HOI)**，實現真正的 End-to-End AI。
...
### 2. 事件層 (Event Extraction) - V4.4 核心突破
*   **動作辨識 (ST-GCN)**：全面導入 **MMAction2 NTU-RGB+D 60** 真實預訓練權重。模型具備強大的時空卷積特徵提取能力，能將 60 種複雜動作零樣本 (Zero-shot) 映射至長照場景，且保留物理啟發式的容錯彈性。
*   **雙軌人機互動 (Dual-HOI)**：
...
- **`V4.4_Architecture_Deep_Dive.md`**: 最完整的系統架構流程圖與資料流。
